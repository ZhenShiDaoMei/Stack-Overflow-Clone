...
export default function FakeStackOverflow() {
  // const [questions, setQuestions] = useState(stackoverflow_data.getAllQstns().sort((a, b) => new Date(b.askDate) - new Date(a.askDate)));
  ...
  const handleSearch =  async(query) => {
    console.log(query)
    const searchQuestions = await axios.get("http://localhost:8000/questions/all?query=" + query)
  ...
  const handleSortActive = () => {
    const sortedQuestions = questions.filter(question => question.answers.length > 0);
    sortedQuestions.forEach(question => {
      const latestAnswerDate = question.answers.map(ansId =>
        stackoverflow_data.data.answers.find(answer => answer.aid === ansId)?.ansDate || new Date(question.askDate)
      ).reduce((latest, current) => new Date(current) > new Date(latest) ? current : latest, question.askDate);
      question.latestActivityDate = latestAnswerDate;
    });

    sortedQuestions.sort((a, b) => new Date(b.latestActivityDate) - new Date(a.latestActivityDate));
    setTempQuestions(sortedQuestions);
  };
  ...
  const handleNewQuestionSubmit = async (question) => {
    ...
    const addQuestion = await axios.post("http://localhost:8000/questions/new", temp)
